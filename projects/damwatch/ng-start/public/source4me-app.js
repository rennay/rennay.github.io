...
$("#btnAddDeal").click(function (event) {
    ...
    if (document.querySelector("#sellerType3").checked) {
    ...
    var xhr = new XMLHttpRequest();
    var url = "https://us-central1-newfriendlychat-63187.cloudfunctions.net/addDeal";
    xhr.open("POST", url, true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            document.querySelector("#twoResponseText").innerHTML = `${now} -- ${xhr.responseText}`;
        }
    };
    var data = JSON.stringify(json);
    xhr.send(data);
...
$("#btnAddUser").click(function (event) {
    ...
    var xhr = new XMLHttpRequest();
    var url = "https://us-central1-newfriendlychat-63187.cloudfunctions.net/addUser";
    xhr.open("POST", url, true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            var now = moment().utcOffset(120);
            document.querySelector("#oneResponseText").innerHTML = `${now} -- ${xhr.responseText}`;
        }
    };
    var data = JSON.stringify(json);
    xhr.send(data);
...
$("#btnGetDeals").click(function (event) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                var selectDeals = document.querySelector("#selectDeals");
                var i;
                for (i = selectDeals.options.length - 1; i >= 0; i--) {
                    selectDeals.remove(i);
                }
                ...
                for (x in myObj.deals) {
                    var opt = document.createElement('option');
                    opt.value = myObj.deals[x].dealUID;
                    opt.innerHTML = moment(myObj.deals[x].expiry).fromNow() + " " + myObj.deals[x].description;
                    selectDeals.appendChild(opt);
                }
...
// Event binding using a convenience method
$("#btnGetLeaderboard").click(function (event) {
        console.log("btnGetLeaderboard clicked...");
        const dealUID = document.querySelector("#selectDeals").value;
        ...
        var observer = query.onSnapshot(querySnapshot => {
            ...
            document.getElementById("threeResponseText").innerHTML = txt;
        }, err => {
            console.log(`Encountered error: ${err}`);
        });
    });
...
// Event binding using a convenience method
$("#chkAdvanced").click(function (event) {
    console.log("-- chkAdvanced");

    var chkAdvanced = document.querySelector("#chkAdvanced");
    var tblAdvanced = document.querySelector("#tblAdvanced");

    if (chkAdvanced.checked == true) {
        tblAdvanced.style.display = "inline";
    }
    else {
        tblAdvanced.style.display = "none";
    }
});

$("#radioBuyer").click(function (event) {
    console.log("-- radioBuyer");

    const categoryTable = document.querySelector("#categoryTable");
    categoryTable.style.display = "none";
});

$("#radioSeller").click(function (event) {
    console.log("-- radioSeller");

    const categoryTable = document.querySelector("#categoryTable");
    categoryTable.style.display = "inline";
});
